...
<div id="content-single" class="content group">
    ...
    <div class="hentry hentry-post blog-big group ">
        ...
        <div class="meta group">
            ...
            <p class="categories"><span>In: <a href="{{route('articlesCategory',['categoryAlias'=>$article->category->alias])}}" title="View all posts in {{$article->category->title}}" rel="category tag">{{$article->category->title}}</a></span></p>
            <p class="comments"><span><a href="#comments" title="Comment on This is the title of the first article. Enjoy it.">{{(count($article->comments))?count($article->comments):'0'}} {{Lang::choice('site.comments',count($article->comments))}}</a></span></p>
        </div>
        <!-- post content -->
    ...
    <div id="comments">
        <h3 id="comments-title">
            <span>{{count($article->comments)}}</span> {{Lang::choice('comments',count($article->comments))}}
        </h3>

        @if(count($article->comments)>0)
        @set($comGroup,$article->comments->groupBy('parent_id'))

        <ol class="commentlist group">

            @foreach($comGroup as $k=>$comments )
                @break($k!==0)

                @include(env('THEME').'.articleComment',['comments'=>$comments])
            @endforeach
        </ol>
        @endif
